...
type Props = {
  data: string,
  current: string,
  paymentRemainAmount: string
}

const LoanInfo = ({ data, current, paymentRemainAmount }: Props) => {
  ...
  const infoList: ListInfoType[] = [
    ...
    {
      title: 'Remain Amount',
      value: <NumberFormatter value={paymentRemainAmount} thousandSeparator prefix={`${currency?.symbol} `} />
    },
    {
