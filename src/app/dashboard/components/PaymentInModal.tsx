...
const PaymentInModal = ({
    payment,
    dateType,
    currency,
}: {
    payment: PaymentOnCalendar;
    dateType: GlobalConfigType["dateType"];
    currency: GlobalConfigType["currency"];
}) => {
    const theme = useMantineTheme();
    return (
        <Card shadow="md" padding="xs" radius="xs" maw={rem(150)} style={{ margin: "auto" }}>
            <Card.Section ta="center">
                <Text mt="md" c={theme.primaryColor} fz={rem(25)} fw="bold">
                    {getMonth(payment.date, dateType)}
                </Text>
                <Text mb="xs" size="sm" c={theme.colors.gray[6]}>
                    {formatDate(payment.date, dateType, "Y-M-d")}
                </Text>
            </Card.Section>
