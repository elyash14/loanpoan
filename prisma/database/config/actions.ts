...
export async function saveLoanConfig(formData: FormData): Promise<SaveLoanConfigResponseType> {
    ...
    try {
        ...
        return {
            data: newConfig,
            status: "SUCCESS",
            message: "Loan config saved successfully",
        }
    } catch (error) {
        console.error(error);
        return {
...
export async function saveInstallmentConfig(formData: FormData): Promise<SaveInstallmentConfigResponseType> {
    // validate the form data on the server
    const validatedFields = saveInstallmentConfigSchema.safeParse({
    ...
    try {
        ...
        const newConfig: GlobalConfigType = {
            ...config,
            installment: {
                dueDay: validatedFields.data.dueDay,
                payDay: validatedFields.data.payDay,
            }
        }
        ...
        return {
            data: newConfig,
            status: "SUCCESS",
            message: "Installment config saved successfully",
        }
    } catch (error) {
        console.error(error);
        return {
            status: "ERROR",
            message: "Failed to save installment config",
        }
    }
}